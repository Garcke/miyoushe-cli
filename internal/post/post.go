...
// UnmarshalJSON 容忍 imgs 元素的两种形态（2026-09-20 实测，分区帖子流的双编码
// content 内 imgs 为字符串数组；发布域为对象数组）：字符串记入 URL。
func (i *imgRaw) UnmarshalJSON(b []byte) error {
...
// 发布/草稿域为 {describe, imgs} 对象。
//
// 字符串形态只在符合已知包装结构（至少含 describe/imgs 之一且类型合法）时
// 才二次解码；普通文本、HTML、普通 JSON 文本（如 `{"foo":1}`）原样保留，
// 不得因“以 { 开头且 JSON 合法”就丢成空正文（R1）。
func (c *contentRaw) UnmarshalJSON(b []byte) error {
	...
	if len(s) >= 2 && s[0] == '"' {
		...
		if describe, imgs, ok := parseContentWrapper(v); ok {
			// 覆盖式赋值：不保留旧值、不累计图片。
			c.Describe = describe
			c.Imgs = imgs
			return nil
...
// parseContentWrapper 判定字符串 v 是否是已知的内容包装结构：
// 必须是 JSON 对象，且至少存在 describe / imgs 键之一，已出现的已知键
// 类型符合既有内容契约；未知附加字段忽略。
// 返回 ok=false 时调用方必须保留原始文本（普通 JSON、数组、残缺 JSON、
// 已知键类型不合法等都属于此类）。
func parseContentWrapper(v string) (describe string, imgs []imgRaw, ok bool) {
	...
	if hasDescribe {
		if err := json.Unmarshal(rawDescribe, &describe); err != nil {
			return "", nil, false
		}
	}
	if hasImgs {
		if err := json.Unmarshal(rawImgs, &imgs); err != nil {
			return "", nil, false
		}
	}
	return describe, imgs, true
}
